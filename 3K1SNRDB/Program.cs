...
internal static class Program
{
    ...
    private static void AddFriends()
    {
        ...
        client.GetDatabase(Helper.CnnVal())
    ...
    {
        var us = Controller.GetAllUsers();
        Neo4jCRUD ndb = new Neo4jCRUD();
        //foreach (var u in us)
        //    foreach (var f in u.friends_id)
        //        ndb.AddFriend(u.id, f);
        //List<string> output = new List<string>();
        //foreach (var uTo in us.Take(3))
        //    output.Add($"({us[3].login}) -{Controller.GetDistanceToUser(us[3].id, uTo.id)}->({uTo.login})");

        //var r = Controller.GetDistanceToUser(us[3].id, us[2].id);
